...
- 正式包内置 Chocola、Vanilla 与 DeepSeek 三个 PSD 模型，可在「模型设置」中切换。
...
当前版本只提供文字 AI 对话，尚未集成 RVC 声音模型。后续计划增加可选的本地语音链路：先把 AI 回复转换为基础语音，再交给用户自行导入的 RVC 模型进行音色转换，并提供开关、模型管理、缓存清理和失败时回退到文字的机制。
